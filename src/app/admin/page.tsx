    ...
    <div dir="rtl" className="min-h-screen flex items-center justify-center px-4" style={{ background: "#05050f" }}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-extrabold text-[#7C6FFF]">پنل مدیریت ماهیر</h1>
        </div>
        <div className="rounded-2xl p-7" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)}
            ...
            className="w-full rounded-xl px-4 py-3 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-[#7C6FFF]/50"
    ...
    <div dir="rtl" className="min-h-screen" style={{ background: "#05050f", color: "#f0f0f5" }}>
      <nav style={{ background: "rgba(5,5,15,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between px-6 py-4">
          <h1 className="text-lg font-extrabold text-[#7C6FFF]">🔐 پنل ادمین ماهیر</h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="rounded-2xl p-5 text-center" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              <div className="text-2xl font-extrabold text-[#7C6FFF]">{s.value}</div>
                  ...
                  <div key={u.id} className="flex items-center justify-between px-6 py-4 hover:bg-white/[0.02] transition-colors">
                    <div className="flex items-center gap-4">
                      <span className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
                        style={{ background: "rgba(124,111,255,0.12)", color: "#7C6FFF" }}>{i + 1}</span>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs px-2 py-1 rounded-full" style={{ background: "rgba(124,111,255,0.1)", color: "#7C6FFF" }}>
        ...
        {tab === "chat" && (
          <div className="flex gap-4 h-[500px]">
            ...
            <div className="w-48 flex-shrink-0 rounded-2xl overflow-hidden flex flex-col" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
              <div className="px-3 py-2.5 text-xs font-bold text-[#7C6FFF]" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>کاربران</div>
              <div className="overflow-y-auto flex-1">
                {users.map(u => (
                  <button key={u.phone} onClick={() => { setSelectedPhone(u.phone); fetchMessages(); }}
                    ...
                    style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", background: selectedPhone === u.phone ? "rgba(124,111,255,0.08)" : "transparent", color: selectedPhone === u.phone ? "#7C6FFF" : "rgba(240,240,245,0.6)" }}>
            ...
            <div className="flex-1 flex flex-col rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.08)" }}>
              {!selectedPhone ? (
                ...
                <>
                  <div className="px-4 py-3 flex items-center gap-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)", background: "rgba(124,111,255,0.05)" }}>
                    ...
                    {chatMessages.length === 0 ? (
                      ...
                      <div key={msg.id} className={`flex ${msg.from === "admin" ? "justify-start" : "justify-end"}`}>
                        <div className="max-w-[70%] rounded-2xl px-4 py-2.5"
                          style={{
                            background: msg.from === "admin" ? "rgba(124,111,255,0.12)" : "rgba(255,255,255,0.07)",
                            border: msg.from === "admin" ? "1px solid rgba(124,111,255,0.2)" : "1px solid rgba(255,255,255,0.1)",
                          }}>
                  ...
                  <div className="p-3 flex gap-2" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                    <input value={msgText} onChange={e => setMsgText(e.target.value)}
                      ...
                      className="flex-1 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#7C6FFF]/50"
                      style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", color: "#f0f0f5" }} />
