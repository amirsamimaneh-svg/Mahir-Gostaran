      ...
      <div className="relative w-full max-w-sm text-center">
        <div className="mb-6">
          <div className="w-20 h-20 rounded-full mx-auto flex items-center justify-center text-4xl mb-4"
            style={{ background: "linear-gradient(135deg,rgba(108,92,231,0.2),rgba(108,92,231,0.05))", border: "2px solid rgba(108,92,231,0.3)", boxShadow: "0 0 40px rgba(108,92,231,0.15)" }}>
            🎉
          </div>
          <h2 className="text-2xl font-extrabold text-[#6C5CE7] mb-1">خوش اومدی!</h2>
          <p className="text-sm" style={{ color: "rgba(240,240,245,0.45)" }}>حساب شما با موفقیت ساخته شد</p>
        </div>

        <div className="rounded-2xl p-6 mb-4"
          style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(108,92,231,0.15)", backdropFilter: "blur(20px)" }}>
          <div className="flex items-center gap-3 mb-4 p-3 rounded-xl" style={{ background: "rgba(108,92,231,0.06)" }}>
          ...
          <button onClick={() => router.push("/consult")}
            ...
            style={{ background: "linear-gradient(135deg,#6C5CE7,#5A4BD6)", color: "#111", boxShadow: "0 0 30px rgba(108,92,231,0.3)" }}>
            شروع مشاوره رایگان ✦
          </button>
        </div>
  ...
  return (
    <div dir="rtl" className="min-h-screen flex" style={{ background: "#05050f" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] flex-shrink-0 p-12 relative overflow-hidden"
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
          {[260, 340, 420].map((s, i) => (
            <div key={i} className="absolute rounded-full"
              style={{
                width: s, height: s,
                top: -s / 2, left: -s / 2,
                border: `1px solid rgba(108,92,231,${0.07 - i * 0.02})`,
          ...
          <div className="w-24 h-24 rounded-full flex items-center justify-center font-extrabold text-3xl"
            style={{ background: "linear-gradient(135deg,#6C5CE7,#5A4BD6)", color: "#111", boxShadow: "0 0 60px rgba(108,92,231,0.25)" }}>
            M
          </div>
        </div>

        <Link href="/" className="text-2xl font-extrabold text-[#6C5CE7] relative z-10">ماهیر</Link>

        <div className="relative z-10">
          <p className="text-xs font-bold tracking-widest mb-3" style={{ color: "rgba(108,92,231,0.5)" }}>امکانات</p>
            ...
            {FEATURES.map((f, i) => (
              <div key={i} className="flex items-center gap-3 transition-all duration-500"
                ...
                <span className="w-8 h-8 rounded-lg flex items-center justify-center text-sm"
                  style={{ background: tick % FEATURES.length === i ? "rgba(108,92,231,0.15)" : "rgba(255,255,255,0.04)", border: "1px solid rgba(108,92,231,0.15)" }}>
                  {f.icon}
                </span>
                <span className="text-sm font-medium" style={{ color: tick % FEATURES.length === i ? "#6C5CE7" : "rgba(240,240,245,0.4)" }}>{f.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 relative z-10">
        ...
        {/* Mobile logo */}
        <Link href="/" className="lg:hidden text-2xl font-extrabold text-[#6C5CE7] mb-8">ماهیر</Link>
        ...
        <div className="w-full max-w-[380px]">
            ...
            {(["login", "register"] as const).map(m => (
              <button key={m} onClick={() => { setMode(m); setError(""); }}
                ...
                style={{
                  ...
                  boxShadow: mode === m ? "0 2px 12px rgba(108,92,231,0.25)" : "none",
                }}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <button onClick={submit} disabled={loading}
              ...
              style={{ background: "linear-gradient(135deg,#6C5CE7,#5A4BD6)", color: "#111", boxShadow: "0 0 30px rgba(108,92,231,0.25)" }}>
              {loading
          ...
          <p className="text-xs text-center mt-6" style={{ color: "rgba(240,240,245,0.25)" }}>
            {mode === "login"
              ? <>حساب نداری؟ <button onClick={() => setMode("register")} className="text-[#6C5CE7] font-bold hover:underline">ثبت‌نام کن</button></>
              : <>حساب داری؟ <button onClick={() => setMode("login")} className="text-[#6C5CE7] font-bold hover:underline">وارد شو</button></>
            }
          </p>
        </div>
      </div>
