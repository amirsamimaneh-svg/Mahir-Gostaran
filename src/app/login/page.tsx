    ...
    <div dir="rtl" className="min-h-screen flex items-center justify-center px-4" style={{ background: "#05050f" }}>
      ...
      <div className="relative w-full max-w-sm text-center">
        <div className="mb-6">
          ...
          <h2 className="text-2xl font-extrabold text-[#2563EB] mb-1">خوش اومدی!</h2>
        ...
        <div className="rounded-2xl p-6 mb-4"
          ...
          <button onClick={() => router.push("/consult")}
            ...
            style={{ background: "linear-gradient(135deg,#2563EB,#3D5AE8)", color: "#111", boxShadow: "0 0 30px rgba(79,110,255,0.3)" }}>
            شروع مشاوره رایگان ✦
          </button>
        </div>
  ...
  return (
    <div dir="rtl" className="min-h-screen flex" style={{ background: "#05050f" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] flex-shrink-0 p-12 relative overflow-hidden"
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
          ...
          <div className="w-24 h-24 rounded-full flex items-center justify-center font-extrabold text-3xl"
            style={{ background: "linear-gradient(135deg,#2563EB,#3D5AE8)", color: "#111", boxShadow: "0 0 60px rgba(79,110,255,0.25)" }}>
            M
          </div>
        </div>

        <Link href="/" className="text-2xl font-extrabold text-[#2563EB] relative z-10">ماهیر</Link>
          ...
          <div className="flex flex-col gap-3">
            {FEATURES.map((f, i) => (
              <div key={i} className="flex items-center gap-3 transition-all duration-500"
                ...
                <span className="text-sm font-medium" style={{ color: tick % FEATURES.length === i ? "#2563EB" : "rgba(240,240,245,0.4)" }}>{f.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 relative z-10">
        ...
        {/* Mobile logo */}
        <Link href="/" className="lg:hidden text-2xl font-extrabold text-[#2563EB] mb-8">ماهیر</Link>
        ...
        <div className="w-full max-w-[380px]">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button onClick={submit} disabled={loading}
              ...
              style={{ background: "linear-gradient(135deg,#2563EB,#3D5AE8)", color: "#111", boxShadow: "0 0 30px rgba(79,110,255,0.25)" }}>
              {loading
          ...
          <p className="text-xs text-center mt-6" style={{ color: "rgba(240,240,245,0.25)" }}>
            {mode === "login"
              ? <>حساب نداری؟ <button onClick={() => setMode("register")} className="text-[#2563EB] font-bold hover:underline">ثبت‌نام کن</button></>
              : <>حساب داری؟ <button onClick={() => setMode("login")} className="text-[#2563EB] font-bold hover:underline">وارد شو</button></>
            }
          </p>
        </div>
      </div>
