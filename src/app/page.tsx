...
const t = {
  fa: {
    ...
    heroSub: "ماهیر با ترکیب استراتژی، هویت برند و هوش مصنوعی، مسیر رشد کسب‌وکار شما را هموار می‌کند.",
    heroCta1: "خدمات ما را ببینید",
    heroCta2: "مشاوره رایگان بگیرید",
    servicesTitle: "خدمات", servicesBrand: "ماهیر",
    ...
    aboutDesc: "ماهیر با تیمی از متخصصان استراتژی، طراحی و فناوری، به کسب‌وکارهای ایرانی کمک می‌کند تا با هویتی قوی و استراتژی هوشمند، در بازار رقابتی امروز متمایز شوند. ما به نتیجه اعتقاد داریم، نه فقط به فرآیند.",
    stats: [
      { num: "+۵۰", label: "کسب‌وکار موفق" },
      { num: "+۸۰٪", label: "رشد میانگین" },
      { num: "۳ سال", label: "تجربه" },
    ],
    footerInstagram: "اینستاگرام", footerLinkedin: "لینکدین",
  ...
  en: {
    ...
    heroSub: "Mahir combines strategy, brand identity, and AI to pave the way for your business growth.",
    heroCta1: "See Our Services",
    heroCta2: "Get Free Consultation",
    servicesTitle: "Our", servicesBrand: "Services",
    ...
    aboutDesc: "Mahir's team of strategy, design, and technology experts helps businesses stand out in today's competitive market with a strong identity and smart strategy. We believe in results, not just process.",
    stats: [
      { num: "50+",  label: "Successful Businesses" },
      { num: "80%+", label: "Average Growth" },
      { num: "3 Yrs", label: "Experience" },
    ],
    footerInstagram: "Instagram", footerLinkedin: "LinkedIn",
...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-6 md:px-12 py-4
      bg-[#07071a]/80 backdrop-blur-md border-b border-white/5 animate-slide-down">
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center
      ...
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <div className="animate-blob delay-100 absolute top-[-10%] right-[-5%] w-[500px] h-[500px]
          rounded-full bg-amber-500/12 blur-[120px]" />
        <div className="animate-blob delay-400 absolute bottom-[-10%] left-[-5%] w-[400px] h-[400px]
          rounded-full bg-indigo-600/12 blur-[120px]" />
        <div className="animate-blob delay-700 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        {[...Array(12)].map((_, i) => (
          <span key={i}
            className="absolute w-1 h-1 rounded-full bg-amber-400/30 animate-blob"
            style={{
              top: `${10 + (i * 7) % 80}%`,
              left: `${5 + (i * 13) % 90}%`,
      ...
      {/* Badge */}
      <span className="animate-fade-up delay-100 mb-6 inline-flex items-center gap-2 rounded-full
      ...
      {/* Headline */}
      <h1 className="animate-fade-up delay-200 text-4xl md:text-6xl lg:text-7xl font-extrabold
        leading-tight tracking-tight text-white max-w-3xl mb-4 mx-auto">
        {tx.heroTitle1}{" "}
        <span className="text-amber-400 relative">
          {tx.heroTitle2}
          {/* underline glow */}
          <span className="absolute -bottom-1 left-0 right-0 h-1 bg-amber-400/40 rounded-full blur-sm" />
        </span>
      </h1>

      <p className="animate-fade-up delay-300 text-white/60 text-base md:text-lg max-w-xl mb-10
        leading-relaxed mx-auto">
        {tx.heroSub}
      </p>

      {/* AI Card */}
      <div className="animate-scale-in delay-400 w-full flex justify-center">
        <IdeaCard lang={lang} />
      </div>

      {/* CTA */}
      <div className="animate-fade-up delay-600 mt-10 flex flex-col sm:flex-row gap-4 justify-center">
        <a href="#services" onClick={(e) => { e.preventDefault(); scrollTo("#services"); }}
          className="px-6 py-3 rounded-xl bg-amber-400 text-gray-900 font-bold
            hover:bg-amber-300 hover:scale-105 transition-all text-sm">
          {tx.heroCta1}
        </a>
        <a href="mailto:hello@mahir.ir"
          className="px-6 py-3 rounded-xl border border-white/20 text-white/80
            hover:border-amber-400/50 hover:text-amber-400 hover:scale-105 transition-all text-sm">
          {tx.heroCta2}
        </a>
      </div>

      {/* Scroll indicator */}
      <div className="animate-fade-in delay-900 absolute bottom-8 left-1/2 -translate-x-1/2
  ...
  return (
    <section id="services" className="py-24 px-4 md:px-12 max-w-6xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {tx.services.map((s, i) => (
          <div key={s.title}
            className="group p-6 rounded-2xl bg-white/5 border border-white/10 text-center
              hover:border-amber-400/50 hover:bg-white/8 hover:-translate-y-2
  ...
  return (
    <section id="about" className="py-24 px-4 md:px-12 max-w-4xl mx-auto w-full">
      <div className="rounded-3xl bg-white/5 border border-white/10 p-10 md:p-14 text-center
        hover:border-amber-400/20 transition-colors duration-500">
        <h2 className="text-3xl font-bold text-white mb-4">
          {tx.aboutTitle} <span className="text-amber-400">{tx.aboutBrand}</span>
        </h2>
        <p className="text-white/60 leading-relaxed text-base max-w-2xl mx-auto">{tx.aboutDesc}</p>
