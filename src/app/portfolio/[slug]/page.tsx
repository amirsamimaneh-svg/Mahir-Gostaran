  ...
  if (!project) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "#05050f", color: "#f0f0f5" }}>
      <div className="text-center">
        ...
        <Link href="/portfolio" className="px-6 py-3 rounded-xl font-bold text-sm" style={{ background: "#6C5CE7", color: "#111" }}>
          بازگشت
        </Link>
      </div>
    </div>
    ...
    <div dir={isRtl ? "rtl" : "ltr"} lang={lang}
      ...
      <nav className="fixed top-0 inset-x-0 z-50"
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between px-6 py-4">
          <Link href="/" className="text-xl font-extrabold text-[#6C5CE7] tracking-widest">
            {isRtl ? "ماهیر" : "Mahir"}
          </Link>
          <div className="flex items-center gap-3">
            <button onClick={() => setLang(l => l === "fa" ? "en" : "fa")}
              className="text-xs font-bold px-3 py-2 rounded-lg transition-all hover:text-[#6C5CE7]"
              style={{ background: "rgba(255,255,255,0.06)", color: "rgba(240,240,245,0.5)" }}>
              {isRtl ? "EN" : "فا"}
            </button>
            <Link href="/portfolio"
              className="text-sm font-medium px-4 py-2 rounded-xl transition-all hover:text-[#6C5CE7]"
      ...
      <div className="max-w-3xl mx-auto px-6 pt-32 pb-20">
        ...
        {project.images?.length > 0 && (
          <div className="mb-10">
            <h2 className="font-bold text-lg mb-4 text-[#6C5CE7]">
        ...
        {/* What we did */}
        <div className="mb-10">
          <h2 className="font-bold text-lg mb-5 text-[#6C5CE7]">
