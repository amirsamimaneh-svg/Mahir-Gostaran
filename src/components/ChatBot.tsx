    ...
    <>
      ...
      <button
        ...
        style={{
          ...
          width: 56, height: 56,
          borderRadius: "50%",
          background: open ? "rgba(30,30,50,0.9)" : "linear-gradient(135deg,#fbbf24,#f59e0b)",
          boxShadow: open ? "0 4px 20px rgba(0,0,0,0.4)" : "0 4px 24px rgba(251,191,36,0.5)",
      ...
      <div
        ...
        <div className="flex flex-col h-full rounded-2xl overflow-hidden"
          ...
          <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0"
            style={{ borderBottom: "1px solid rgba(255,255,255,0.07)", background: "rgba(251,191,36,0.05)" }}>
            <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0"
              style={{ background: "linear-gradient(135deg,#fbbf24,#f59e0b)" }}>
              <span className="text-base font-extrabold text-gray-900">M</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-sm text-amber-400 leading-none">
            ...
            {msgs.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? (isRtl ? "justify-start" : "justify-end") : (isRtl ? "justify-end" : "justify-start")}`}>
                <div
                  ...
                  style={m.role === "user"
                    ? { background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.2)", color: "#f0f0f5" }
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                {suggestions.map(s => (
                  <button key={s} onClick={() => send(s)}
                    className="text-right text-xs px-3 py-2 rounded-xl transition-all hover:border-amber-400/40 hover:text-amber-400 text-start"
          ...
          <div className="px-3 py-3 flex-shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="flex gap-2">
              <input
                ...
                disabled={loading}
                className="flex-1 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-amber-400/50 disabled:opacity-50"
