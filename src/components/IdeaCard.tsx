  ...
  return (
    <div dir={isRtl ? "rtl" : "ltr"}
      ...
      style={{
        ...
        boxShadow: "0 8px 48px rgba(0,0,0,0.5), 0 0 80px rgba(108,92,231,0.06)",
      }}>
      ...
      <div className="flex items-center justify-between px-5 py-3.5"
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-[#6C5CE7] tracking-widest">{tx.title}</span>
        </div>
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex gap-2">
          <input
            ...
            disabled={loading}
            className="flex-1 rounded-xl px-4 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-[#6C5CE7]/50 disabled:opacity-50"
          ...
          <div className="flex flex-wrap gap-1.5">
            {cats.map(cat => (
              <button key={cat} onClick={() => handleSubmit(cat)}
                disabled={loading}
                className="text-xs px-3 py-1 rounded-full transition-all hover:border-[#6C5CE7]/60 hover:text-[#6C5CE7] disabled:opacity-40"
        ...
        <div className="min-h-[90px]">
          {loading && (
            <div className="flex items-center gap-3 py-2">
              <svg className="animate-spin w-4 h-4 text-[#6C5CE7] flex-shrink-0" fill="none" viewBox="0 0 24 24">
          ...
          {hasError && !loading && (
            <div className="rounded-2xl p-4"
              ...
              <p className="text-xs font-bold text-[#6C5CE7] mb-2">✦ {tx.errorTitle}</p>
              {tx.errorMsg.split("\n").map((line, i) => (
              ...
              <a href="https://t.me/Mahirofficalll" target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 mt-3 text-sm font-bold text-[#6C5CE7] hover:underline">
                ✈ {tx.tg}
              </a>
            </div>
          )}
          ...
          {!loading && !hasError && displayed && (
            <div className={isRtl ? "text-right" : "text-left"} style={{ padding: "4px 0" }}>
              {displayed.split("\n").map((line, idx) =>
                idx === 0
                  ? <p key={idx} className="font-extrabold text-[#6C5CE7] text-base mb-2">{line}</p>
                  : line.trim()
                    ? <p key={idx} className="text-sm leading-relaxed mb-1" style={{ color: "rgba(240,240,245,0.8)" }}>{line}</p>
                    : <br key={idx} />
              )}
              <span className="inline-block w-0.5 h-4 bg-[#6C5CE7] align-middle ml-0.5"
